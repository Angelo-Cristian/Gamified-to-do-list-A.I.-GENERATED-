# Android Project

## Project Title
Contorizare de activități

## Description
Este o aplicație competitivă de productivitate prin care îți poți organiza activitățiile zilnice și să-ți compari productivitatea cu alții.

## Features
Server c++ cu drogon
Client în android studio cu kotlin și jetpack compose
Realtime database în firebase

...

## Screenshots <img width="921" height="2048" alt="WhatsApp Image 2026-04-15 at 11 52 35" src="https://github.com/user-attachments/assets/512ee5a9-5a89-4edb-8cf6-9e0bd2ac11ce" />
<img width="921" height="2048" alt="WhatsApp Image 2026-04-15 at 11 52 35 (1)" src="https://github.com/user-attachments/assets/1e123542-5a69-4d42-a8c2-48d3109bba0f" />
<img width="921" height="2048" alt="WhatsApp Image 2026-04-15 at 11 52 35 (3)" src="https://github.com/user-attachments/assets/d46c6475-b9e9-4fcf-a139-3c3ac5e415bc" />
<img width="921" height="2048" alt="WhatsApp Image 2026-04-15 at 11 52 35 (2)" src="https://github.com/user-attachments/assets/b48eeefc-16c1-4b2e-a956-4c320a0dbdb4" />

## Tehnologies Used
- Kotlin
- Drogon
- Jetpack Compose
- Realtime database în firebase

...

## How to Run
1. Clonează repository-ul
2. Deschide Android Studio
3. Deschide fișierul the app
4. în vs2022 - open folder - server (din repository)
